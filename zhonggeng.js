...
[task_local]
#中庚签到
10 6 * * * https://github.com/glenq/autotask/zhonggeng.js, tag=中庚签到, enabled=true
...
async function doSign(userid) {
    try {
        ...
        if (result.resCode == 10024){
            console.log(`中庚签到成功，结果：${result}`)
        }else {
            console.log(`中庚签到失败，错误数据${result}`)
        }
    } catch (e) {
        $.logErr(e)
    }
